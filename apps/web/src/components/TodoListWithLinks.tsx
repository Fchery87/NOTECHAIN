...
/**
 * Props for TodoListWithLinks component
 */
export interface TodoListWithLinksProps {
  todos: Todo[];
...
/**
 * TodoList component with Note-To-Todo deep linking
 * FR-TODO-01: Create, edit, delete tasks
 * US-2.2: Todo from Note Context - visual indicators and deep linking
 * US-2.2 AC4: Todo list displays distinct link icon for linked todos
 * US-2.2 AC2: Clicking link icon opens source note
 */
export function TodoListWithLinks({
  todos,
  onToggle,
  onPress,
  onDelete,
  ...
  return (
    <div className="space-y-2" role="list" aria-label="Todo list">
      {todos.map(todo => {
        ...
        const hasLinkedNote = !!todo.linkedNoteId;
                ...
                <div className="flex items-center gap-2">
                  ...
                  {hasLinkedNote && (
                    <button
                      onClick={e => {
                        e.stopPropagation();
                        if (onOpenLinkedNote && todo.linkedNoteId) {
                          onOpenLinkedNote(todo.linkedNoteId);
                        }
                      }}
                      className="p-1 text-amber-600 hover:text-amber-700 hover:bg-amber-50 rounded-lg transition-colors"
                      title="Linked to note"
...
export default TodoListWithLinks;
